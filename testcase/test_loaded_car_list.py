...
class LoadedListTest(StartEnd):
    ...
    def test_loaded_list(self):
        ...
        if result not in expect:
            self.save_screenshot('已装车列表页面断言失败')
        self.assertIn(result, expect, msg='已装车列表页面测试失败，实际值{}不在期望值{}中'.format(result, expect))
        Log().info('----------已装车列表页面测试通过----------')
        Log().info('----------测试完成----------')
